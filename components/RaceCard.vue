<template>
  <!-- Course passée : lien vers les résultats. Course à venir : bloc non interactif. -->
  <component
    :is="isPastRace ? NuxtLink : 'div'"
    :to="isPastRace ? `/races/${race.season}/${race.round}` : undefined"
    class="group relative block bg-f1-dark-gray/40 backdrop-blur-md rounded-2xl border border-white/5 overflow-hidden transition-all duration-300"
    :class="isPastRace
      ? 'hover:border-f1-red/50 hover:shadow-[0_0_20px_rgba(255,24,1,0.15)] hover:-translate-y-1 opacity-75 hover:opacity-100 focus-visible:opacity-100'
      : ''"
  >
    <!-- Hover Gradient -->
    ...
    <div class="hidden md:block p-6 relative z-10">
      <div class="flex justify-between items-start mb-4">
        <div class="flex flex-col">
          ...
          <h2
            class="text-xl font-display font-bold italic text-white leading-tight transition-colors"
            :class="{ 'group-hover:text-f1-red': isPastRace }"
          >
            {{ race.raceName }}
          </h2>
        </div>
        ...
        <div v-else class="px-2 py-1 rounded text-xs font-bold bg-f1-red-action text-white shadow-lg shadow-f1-red/20 animate-pulse-slow">
          À venir
        </div>
      </div>
